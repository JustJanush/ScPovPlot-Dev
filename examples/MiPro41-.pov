...
#local _FileVerMaj = "3.3.0.0";
#local _FileVerMin = "2018-01-05";
...
#declare BoxMin = _MoleculeCenter+ 2.0*(BoxMin-_MoleculeCenter);
//                                 ^----- tune these coeeficients manually!
#declare BoxMin =  <BoxMin.x, 3*BoxMin.y, BoxMin.z>; // ***
//                            ^----- tune this coeficient manually!
//------------------------------------------------------------------<<
#declare BoxMax = _MoleculeCenter+ 2.0*(BoxMax-_MoleculeCenter);
...
#declare N2  = 60;
#declare N3  = 16;
#declare dd  = .3; //cell size
#declare dd3 = pow(dd,3);
#declare rr  = .2*dd;
#declare rc  = rr/3;
#local _dst  = 20; // camera distance 

#local _T  = 0;//23094.0*sin(pi*pow((2*clock-1),5)/3); //+20000*pow(clock,3);
...
#local gMin  = 0;//-3.5;
#local gMin2 = 0;
#local gMax  = 35E3; // (1-clock)*5E3+clock*5E4; // 5E7;

//SetRGBMap(RGB4bw)
SetRGBMap(RGB4a)
SetGradientAccuracy(.001) // .0001)

#local VVV  = VVV_C2; // VVV  is function pointer. Now it is assigned a job to do.
...
////////////    
    #case ( _IsoPot )
       ...
       #local _Ts = concat("IsoPot. const(#|-",_nabla_,"V(r)#|), V(r) ",_inSet_," [",TrimStr(str(gMin/1000,8,1)),_fromTo_, TrimStr(str(gMax/1000,8,1)), "]||[V]");//"Isosurface colored by gradient";
...
///////////
    #case (_InterPot)
        ...
        #local _Ts   = concat("InterPot, N. of cells= ",TrimStr(str(dimension_size(QVr,1)*dimension_size(QVr,2)*dimension_size(QVr,3), 10, 0)),";  #|-",_nabla_,"V(r)#|= ", _Ts );
        #debug _Ts
       ...
       #debug concat(" [", str(_CubicPotInter, 1, 0), "] - Isosurface of potential textured by field strength.\n")
        ...
        #for(in1,0,dimension_size(QVr,1)-1) 
           #for(in2,0,dimension_size(QVr,2)-1)
              #for(in3,0,dimension_size(QVr,3)-1)
                    // real coordinates!!
                    #local xxx = in1*dd-BoxMax.z-1.5+_rdx;
                    #local yyy = in2*dd+BoxMin.x-1.5+_rdy;
                    #local zzz = in3*dd+BoxMax.y-4.5-dd+_rdz;
//                    #local rrV = xxx*xxx+yyy*yyy+zzz*zzz;
...
//        #local gMax = 1.8E6;
        ...
        #local _Ls   = TrimStr(str(gMax/1000,8,1));
        #local _Ts   = concat(TrimStr(str(gMin/1,5,1)), _fromTo_, _Ls,"|[V/",_Ao_,"] ");
        #local _Ts   = concat(" CubicPot, V_0=", TrimStr(str(_T,7,1)), "; N. of cells= ",TrimStr(str(dimension_size(QVr,1)*dimension_size(QVr,2)*dimension_size(QVr,3), 10, 0)),";  #|-",_nabla_,"V(r)#|= ", _Ts );
        #debug _Ts
...
// #declare BoxMin =  <BoxMin.x, 2.2*BoxMin.y, BoxMin.z>; // ***
    ...
    SetRGBMap(RGB4a)
    // SetRGBMap(RGBstp)
    SetGradientAccuracy(.005) //.0001)

    #local grV0 = fn_Gradient(VVV);     // *** VVV
    #local grV = function{clip(VVV(x,y,z)-gMin+1, 0, gMax-gMin+1)/(gMax-gMin+1)};
    SetIsoTexture( texture{
       pigment{
         function{ grV(x,y,z) }
         color_map{_RGBMap}
       ...
       finish{ Dull } // *** scDullMirror }
        ...
        #local _T  = 1000.0;
        #local _Ts = concat("const(#|-",_nabla_,"V(r)#|), V(r) ",_inSet_," [", TrimStr(str(gMin/1000,8,1)),_fromTo_, TrimStr(str(gMax/1000,8,1)), "]||[V]");
...
/////////////
    #case (_InterGrad)
       #debug concat(" [", str(_InterGrad, 1, 0), "] Isosurface colored by gradient, interpolated version.\n")
...
#ifdef(_dstx)
    #local dst_ = _dstx;  // specificx distance from observed point
#else
